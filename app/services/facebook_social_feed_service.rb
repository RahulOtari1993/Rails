class FacebookSocialFeedService
  ...
  def process
    if @organization.present? && @campaign.present? && @network.present?
      ...
      facebook_media_challenge = @campaign.challenges.featured.current_active.where(challenge_type: 'engage', parameters: 'facebook').first

      ## fetch client auth token and facebook app secret
      auth_token = @network.auth_token
      facebook_app_secret = (@campaign.present? && @campaign.white_branding) ? CampaignConfig.where(campaign_id: @campaign.id).first.facebook_app_secret  : GlobalConfiguration.first
      ...
      @page_records.each do |page_record|
        ...
        network_page = @campaign.network_pages.where(network_id: @network.id, page_id: page_record['id']).first
